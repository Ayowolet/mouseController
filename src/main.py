...
def get_args():
    ...
    option.add_argument("-vf", nargs='+', help="specify flags from mfd, mld, mhp, mgd e.g. -vf mfd mld mhp mgd (seperate each flag by space) for visualization of the output of intermediate models", default=[], required=False)
...
def model_pipelines(args):
    ...
    for _, frame in input_feed.next_batch():
        ...
        croppedFace = pipeline_faceDetection.predict(frame)
        face_inference_time = time.time() - face_inference_start_time

        if 'mfd' in visual_flag:
            
            cv2.imshow('The cropped face', croppedFace)
        ...
        eye_image_left, eye_image_right, face_landmarked = pipeline_landmarksDetection.predict(croppedFace.copy())
        ...
        head_pose_angles, himage = pipeline_headPoseEstimation.predict(croppedFace.copy())   
